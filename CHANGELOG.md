...
- Поддержка запуска CLI на самом устройстве (Termux/Pydroid): деплой без
  ...
  `adb`). См. ADR-0011.
- `catalib watch` работает **без** `watchfiles`: при отсутствии пакета —
  stdlib-поллинг (интервал `--poll`, по умолчанию 1 с). Активный бэкенд
  печатается при старте. Важно для Termux/Pydroid, где Rust-бэкенд не
  собрать.
- `catalib doctor` адаптируется под устройство: печатает среду (ПК или
...
- `catalib watch` больше не завершается с ошибкой при отсутствии
  `watchfiles` (ревизия ADR-0005): пакет остаётся опциональным и
  предпочтительным (мгновенный, дешевле по CPU), но не обязателен.
...
- Модуль SDK `intents` решено **не оборачивать**: его нет статически в
  ...
  SDK, поверхность не зафиксирована. Эмпирическая проверка зондом на
  живом устройстве подтвердила: `intents.so` без документации, реальный
  API — один класс `IntentsManager` (перехват Android-интентов), узкая
  ниша. Обёртка по догадке противоречит ADR-0006/0007 и не тестируема
  офлайн. Прямой импорт на устройстве по-прежнему доступен (book —
  «Доступ к SDK» с реальной точкой входа `IntentsManager` и оговоркой).
  См. ADR-0010.
