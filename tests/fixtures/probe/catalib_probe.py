...
def _sdk_log(message: str) -> None:
    """Записать строку в лог приложения, если SDK доступен, иначе в stdout."""
    try:
    ...
    try:
        record(
            "module_identity",
            {
                ...
                "self_module_file": getattr(
                    sys.modules.get(__id__, None), "__file__", None
                ),
            },
        )
    except Exception as exc:
        ...
        record(
            "import_state",
            {
                ...
                "related_modules": sorted(
                    k for k in sys.modules if __id__ in k or _PREFIX in k
                ),
            },
        )
    except Exception as exc:
...
def _emit(result: dict) -> str:
    """Записать результат в JSON рядом с плагином и вернуть путь."""
    text = json.dumps(result, ensure_ascii=False, indent=2)
    out_path = None
    try:
        from file_utils import get_plugins_dir

        out_path = os.path.join(get_plugins_dir(), "catalib_probe_result.json")
        with open(out_path, "w", encoding="utf-8") as handle:
            handle.write(text)
    except Exception as exc:
        _sdk_log(f"[catalib_probe] не удалось записать JSON: {exc}")
...
class CatalibProbePlugin(BasePlugin):
    ...
    def on_plugin_load(self) -> None:
        try:
            ...
            path = _emit(result)
            self.log(f"[catalib_probe] диагностика записана: {path}")
        except Exception as exc:
            self.log(f"[catalib_probe] фатальная ошибка зонда: {exc}")
