...
## Результат валидации T-081 (2026-05-17, устройство NX789J)

Шаблон `catalib init "Greet" --id greet` собран `catalib build` и доставлен
через dev server. `get_plugins` вернул для `greet` **`error: null`** — это
значит, что exteraGram под Chaquopy успешно импортировал собранный файл, то
есть отработали встроенный загрузчик на `sys.meta_path`, вендоренный
`catalib.support`, относительный импорт подмодуля `.greeting` и
`CatalibPlugin`. Критерий «плагин загружается на реальном устройстве»
выполнен. Это дополняет зонд T-011 (механизм) и сквозной subprocess-тест.

Наблюдаемые особенности dev server на этой сборке (не относятся к catalib):

- `set_plugin_enabled` подтверждается, но `enabled` остаётся `false` —
  включение, по-видимому, требует UI экрана плагинов exteraGram;
- `delete_plugin` подтверждается, но файл не удаляется — приватный каталог
  без root недоступен для `adb`; удалять плагин надёжнее через UI exteraGram.
