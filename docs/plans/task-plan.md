...
### T-108: Релиз 0.2.0
- **Статус:** blocked (нет учётных данных для push/PyPI; ожидается ввод
  пользователя — см. примечание)
...
- **Примечание:** выполнено — версия 0.2.0, CHANGELOG `[0.2.0] - 2026-05-18`,
  самопроверка зелёная (ruff, 197 тестов, `python -m build`, `twine check`),
  релиз закоммичен и помечен тегом `v0.2.0` локально, артефакты собраны.
  Не выполнено (нет учётных данных в окружении): `git push origin main` и
  тега (HTTPS-remote без креденшелов, нет `gh`/`GITHUB_TOKEN`/credential
  helper; SSH-ключ есть, но не авторизован для репозитория) и публикация в
  PyPI (нет `~/.pypirc`/`TWINE_*`). Остановлено и задан вопрос пользователю,
  как именно предоставить доступ.
