...
- **Зависит от:** T-212, T-213, T-214, T-215, T-216, T-218
- **Блокирует:** —

### T-218: Проверка обновлений catalib через PyPI
- **Статус:** done
- **Описание:** функция `check_for_updates()` в `catalib/__init__.py`:
  запрос `https://pypi.org/pypi/catalib/json` (urllib, короткий таймаут,
  ленивый импорт — не при import пакета), сравнение версий стандартной
  библиотекой; кеш результата `~/.cache/catalib/update-check.json` (TTL
  сутки); полностью безопасна (любая ошибка/таймаут — молча `None`);
  opt-out `CATALIB_NO_UPDATE_CHECK=1`. Вызов — из CLI (`main`/`version`),
  одна строка-уведомление в stderr, никогда не валит команду. Не
  вызывается при импорте (вендоренный `catalib` в плагинах не делает
  сетевых запросов).
- **Артефакты:** `src/catalib/__init__.py`, `src/catalib/cli/app.py`,
  `tests/unit/test_update_check.py`, `docs/components/cli.md`,
  `book/cli/overview.md`, `CHANGELOG.md`.
- **Критерий завершения:** opt-out/ошибка сети/кеш/обнаружение новой
  версии покрыты тестами; CLI не падает при сбое проверки; тесты зелёные.
- **Зависит от:** —
- **Блокирует:** T-217.
...
- **Статус:** done (релиз — по запросу пользователя)
