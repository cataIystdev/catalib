...
### T-011: Деплой зонда на устройство и снятие логов
- **Статус:** pending
- **Описание:** push зонда на `NX789J` через ADB, включение, чтение логов,
  фиксация фактического поведения движка.
- **Артефакты:** дополнение к ADR-0002 с результатом (раздел «Эмпирическое
  подтверждение»).
- **Критерий завершения:** подтверждено (или опровергнуто с выбором запасного
  механизма), что кастомный `sys.meta_path` работает в Chaquopy.
- **Зависит от:** T-010
...
### T-060: ADB-обёртка
- **Статус:** pending
- **Описание:** список устройств, push в каталог плагинов, проверка наличия
  `adb`, флаг `--serial`, безаргументные shell-инъекции исключены.
- **Артефакты:** `src/catalib/deploy/adb.py`.
- **Критерий завершения:** команды формируются корректно (мок подпроцесса).
- **Зависит от:** T-003
- **Блокирует:** T-061, T-062.

### T-061: Перезагрузка плагина на устройстве
- **Статус:** pending
- **Описание:** деплой `<plugin_id>.py` и инициирование перезагрузки плагина.
- **Артефакты:** `src/catalib/deploy/reload.py`.
- **Критерий завершения:** последовательность шагов корректна (мок).
