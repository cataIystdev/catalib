...
- Деплой на устройство через ADB (`adb push` в каталог плагинов) и перезагрузка.
...
graph LR
  subgraph DEV[Машина разработчика]
    ...
    CLI --> DEP[deploy: ADB push + reload]
    OUT --> DEP
  end
  subgraph DEVICE[Устройство]
    DEP --> PDIR["/data/.../files/plugins/&lt;plugin_id&gt;.py"]
...
| Деплой | внешний бинарь `adb` | — | Единственный надёжный канал на устройство; вызывается как подпроцесс. |
...
- E2E на устройстве не автоматизируется (зависимость от железа), но
  поддерживается ручной сценарий через зонд (T-301) и `catalib watch --deploy`.
...
|----------------|-------------|---------|-----------|
| Chaquopy ограничивает `sys.meta_path` или импорт-хуки | Низкая | Критическое | Зонд на устройстве (T-301) до реализации ядра; запасной механизм — регистрация модулей напрямую в `sys.modules` через `exec` исходников. |
...
(T-301) → ядро bundler → CLI → хелперы → тесты. Зонд выполняется максимально рано,
