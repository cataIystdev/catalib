...
Посмотреть логи с устройства, отфильтрованные по плагину:
[`catalib logs`](../cli/logs.md). Учтите: в некоторых сборках логи
плагина **не доходят до `adb logcat`** — тогда для диагностики надёжнее
писать в файл в каталоге плагина (см.
...
`intents` — ровно такой случай. catalib его **намеренно не оборачивает**:
модуля нет статически в APK (приложение даёт его в рантайме), он
отсутствует в выверенном справочнике SDK, а поверхность не зафиксирована
и может меняться между сборками exteraGram (см. ADR-0010). Обёртка по
догадке была бы хрупкой и не тестируемой офлайн.

Снятая с устройства реальная поверхность (для ориентира, не контракт):
модуль — компилированный `intents.so`, без docstring и `__all__`. Почти
всё в его неймспейсе — реэкспорт: `Intent`/`Uri` это
`android.content.Intent`/`android.net.Uri`, остальное — Java-классы
через `jclass` и утянутые импорты. Собственная точка входа одна:

> **`intents.IntentsManager`** — глобальный перехват Android-интентов:
> `new_global_before_handler`, `new_global_after_handler`, `parse`,
> `unhandle` (+ вложенные `HandlerHandle`, `HandlerInfo`,
> `HandlerNotRegistered`).

Если это действительно нужно — импортируйте напрямую, со страховкой и за
адаптером:
...
Сигнатуры методов не интроспектируются (`.so`), документации нет и
поведение может меняться между сборками: проверяйте наличие атрибутов
перед вызовом, а офлайн-тесты держите на своём адаптере, не на самом
`intents`.
